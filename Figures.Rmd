...
elm_categories <- c("Cleavage", "Degrons","Substrate\nrecognition", "Scaffolding", "PTM sites", "Localization")
...
BRET_results <- BRET_results %>% mutate(pval_bret50 = as.double(gsub("\\,","\\.",pval_bret50)) ) %>% 
...
BRET_results <- BRET_results %>% mutate(BRET_category=case_when(num_use_fit_WT==3 & num_use_fit_MUT==3 ~ "BRET50", 
                                                                ...
                                                                TRUE ~ "other"))
BRET_results$tstat_cBRET <- as.double(gsub("\\,","\\.",BRET_results$tstat_cBRET))
BRET_results$tstat_bret50 <- as.double(gsub("\\,","\\.",BRET_results$tstat_bret50))

domain_BRET50 <- BRET_results %>% filter(mut_type=="Domain validation" & BRET_category=="BRET50") %>% 
  mutate(mutant=gsub("^pc.*?NL-", "", NL_plasmid))
domain_maxBRET <- BRET_results %>% filter(mut_type=="Domain validation" & BRET_category=="maxBRET") %>% 
  mutate(mutant=gsub("^pc.*?NL-", "", NL_plasmid))
domain_BRET <- BRET_results %>% filter(mut_type=="Domain validation") %>% 
  mutate(mutant=gsub("^pc.*?NL-", "", NL_plasmid))

motif_BRET50 <- BRET_results %>% filter(mut_type=="Motif validation" & BRET_category=="BRET50") %>% 
  mutate(mutant=gsub("^pc.*?His3C-", "", mCit_plasmid))
motif_maxBRET <- BRET_results %>% filter(mut_type=="Motif validation" & BRET_category=="maxBRET") %>% 
  mutate(mutant=gsub("^pc.*?His3C-", "", mCit_plasmid))

mutants_BRET50 <- BRET_results %>% filter(exp_category=="variant" & BRET_category=="BRET50") %>% 
  mutate(domain_variant=str_detect(NL_plasmid, "_[A-Z][0-9]{1,4}[A-Z]"), motif_variant=str_detect(mCit_plasmid, "_[A-Z][0-9]{1,4}[A-Z]")) %>%
  mutate(mutant=case_when(domain_variant==TRUE ~ gsub("^pc.*?NL-", "", NL_plasmid), motif_variant==TRUE ~ gsub("^pc.*?His3C-", "", mCit_plasmid) )) %>% 
  mutate(mutant=case_when(domain_variant==TRUE ~ gsub("_", "\n",mutant), motif_variant==TRUE ~ gsub("_", " ",mutant)))
mutants_maxBRET <- BRET_results %>% filter(exp_category=="variant" & BRET_category=="maxBRET") %>%
  mutate(domain_variant=str_detect(NL_plasmid, "_[A-Z][0-9]{1,4}[A-Z]"), motif_variant=str_detect(mCit_plasmid, "_[A-Z][0-9]{1,4}[A-Z]"))%>%
  mutate(mutant=case_when(domain_variant==TRUE ~ gsub("^pc.*?NL-", "", NL_plasmid), motif_variant==TRUE ~ gsub("^pc.*?His3C-", "", mCit_plasmid) )) %>% 
  mutate(mutant=case_when(domain_variant==TRUE ~ gsub("_", "\n",mutant), motif_variant==TRUE ~ gsub("_", " ",mutant)))

mutants_BRET50 %>% count(NL_symbol, motif_variant, domain_variant)
mutants_maxBRET %>% count(NL_symbol, motif_variant, domain_variant)

domain_BRET50 %>% filter(NL_symbol=="CTBP1") %>% select(tstat_bret50)

BRET_results %>% count(BRET_category)
colnames(BRET_results)
```
...
UB_motif_degrons <- HURI_ClinVar_mut_motif_summ %>% filter(structural_region=="m_interface" & ELM_type=="DEG" & pathogenicity=="Uncertain" & variant_match==0) %>% unique()
...
```{r Figure1_Graphic_D}
p1 <- ggplot(RRS4_ROC, aes(x=mean_fpr, y=mean_tpr)) +  geom_line() + 
  ...
  theme_classic() + theme(text = element_text(size = 18), axis.text = element_text(color = "black"),
                          axis.line = element_line(size = 1), axis.ticks = element_line(linewidth = 1)) + 
...
p2 <- ggplot(RRS4_PRC, aes(x=mean_recall, y=mean_precision)) +  geom_line() + 
...
# Heatmap
heatmap_pltt <- 'RdBu'
p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_bret50  )) + geom_tile(color = "white", lwd = 1.5) + geom_text(aes(label=sig_BRET50)) +
  ...
  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        ...
        legend.title = element_text(size=12), legend.key.height= unit(0.7, 'cm'), legend.key.width= unit(0.35, 'cm'), legend.text = element_text(size=10, vjust=1)) +
  scale_y_discrete(limits=rev)

p2 <- ggplot(domain_BRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_cBRET  )) + geom_tile(color = "white", lwd = 1.5) + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  xlab("CTBP1 domain mutants") + 
  theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(),
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(),
        axis.text.x = element_text(angle = 30, vjust = 1, hjust=1), 
        legend.title = element_text(size=12), legend.key.height= unit(0.7, 'cm'), legend.key.width= unit(0.35, 'cm'), legend.text = element_text(size=10, vjust=1)) +
  scale_y_discrete(limits=rev)
...
p1 <- ggplot(domain_BRET50 %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_bret50  )) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
...
p2 <- ggplot(domain_maxBRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
...
grid.arrange(p1, p2,nrow=2, heights = c(1,2))

# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/DomainValidation_CTBP1.tiff", units="cm", width=10, height=10, res=800)
grid.arrange(p1, p2,nrow=2, heights = c(1,2))
dev.off()

p1 <- ggplot(domain_BRET50 %>% filter(NL_symbol=="SPOP") %>% mutate(mutant=gsub("SPOP_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_bret50  )) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + xlab("SPOP domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank(),
...
p2 <- ggplot(domain_maxBRET %>% filter(NL_symbol=="SPOP") %>% mutate(mutant=gsub("SPOP_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-31, 31), na.value = "grey50")+
  ggtitle("maxBRET") + xlab("SPOP domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank()) +
...
grid.arrange(p1, p2,nrow=2, heights = c(4,3))

# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/DomainValidation_SPOP.tiff", units="cm", width=10, height=10, res=800)
grid.arrange(p1, p2,nrow=2, heights = c(4,3))
dev.off()



p1 <- ggplot(domain_BRET50 %>% filter(NL_symbol=="WWOX") %>% mutate(mutant=gsub("WWOX_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_bret50  )) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + xlab("WWOX domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank(),
...
p2 <- ggplot(domain_maxBRET %>% filter(NL_symbol=="WWOX") %>% mutate(mutant=gsub("WWOX_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("maxBRET") + xlab("WWOX domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(),
...
grid.arrange(p1, p2, nrow=2, heights = c(3,5))

# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/DomainValidation_WWOX.tiff", units="cm", width=10, height=16, res=800)
grid.arrange(p1, p2, nrow=2, heights = c(3,5))
dev.off()


p1 <- ggplot(domain_BRET50 %>% filter(NL_symbol=="PPP3CA") %>% mutate(mutant=gsub("PPP3CA_", "", mutant)) , aes(mutant, mCit_symbol, fill=tstat_bret50  )) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + xlab("PPP3CA\ndomain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(),
...
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/DomainValidation_PPP3CA.tiff", units="cm", width=8, height=8, res=800)
p1
dev.off()


p1 <- ggplot(domain_BRET50 %>% filter(NL_symbol=="IQCB1") %>% mutate(mCit_plasmid=gsub("_", "\n", gsub("^.*?His3C-", "", mCit_plasmid))) , aes(mCit_plasmid, NL_symbol, fill=tstat_bret50)) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + xlab("Domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank()) +
...
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/DomainValidation_IQCB1.tiff", units="cm", width=8, height=5, res=800)
p1
dev.off()
...
```{r Figure3_Graph_motif}
# Heatmaps

p1 <- ggplot(motif_BRET50 %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), aes(NL_symbol, mutant, fill=tstat_bret50)) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank()) +
...
p2 <- ggplot(motif_maxBRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), aes(NL_symbol, mutant, fill=tstat_cBRET)) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("maxBRET") + xlab("CTBP1 domain mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
grid.arrange(p1, p2,nrow=2, heights = c(2, 2), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/MotifValidation_CTBP1.tiff", units="cm", width=8, height=8, res=800)
grid.arrange(p1, p2,nrow=2, heights = c(2, 2), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
dev.off()


p1 <- ggplot(motif_BRET50 %>% filter(NL_symbol=="SPOP") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), aes(NL_symbol, mutant, fill=tstat_bret50)) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif mutants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
p2 <- ggplot(motif_maxBRET %>% filter(NL_symbol=="SPOP") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), aes( NL_symbol, mutant, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-60, 60), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif deletions") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.x=element_blank(), axis.title.y=element_blank()) +
  scale_y_discrete(limits=rev)

grid.arrange(p1, p2,nrow=2, heights = c(2, 1), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/MotifValidation_SPOP.tiff", units="cm", width=8, height=12, res=800)
grid.arrange(p1, p2,nrow=2, heights = c(2, 1), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
dev.off()


p1 <- ggplot(motif_BRET50 %>% filter(NL_symbol=="WWOX") %>% mutate(mutant=gsub("_", " ", gsub("_([0-9]{1,3})_", "\n\\1-", mutant))), aes( NL_symbol, mutant, fill=tstat_bret50  )) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-41, 41), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif deletions") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 18), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank()) +
  scale_y_discrete(limits=rev)

p2 <- ggplot(motif_maxBRET %>% filter(NL_symbol=="WWOX") %>% mutate(mutant=gsub("_", " ",gsub("_([0-9]{1,3})_", "\n\\1-", mutant))), aes( NL_symbol, mutant, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-41, 41), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif deletions") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 18), panel.grid.major = element_blank(), legend.position = "none",
...
grid.arrange(p1, p2, nrow=2, heights = c(1, 2), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/MotifValidation_WWOX.tiff", units="cm", width=8, height=16, res=800)
grid.arrange(p1, p2, nrow=2, heights = c(1, 2), left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90 ))
dev.off()
...
p2 <- ggplot(motif_maxBRET %>% filter(NL_symbol=="PPP3CA") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), aes( NL_symbol, mutant, fill=tstat_cBRET  )) + geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-21, 21), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif deletion") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 18), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.x=element_blank()) +
  scale_y_discrete(limits=rev)
p2
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/MotifValidation_PPP3CA.tiff", units="cm", width=7, height=5, res=800)
p2
dev.off()


p1 <- ggplot(motif_BRET50 %>% filter(NL_symbol=="IQCB1") %>% mutate(NL_plasmid=gsub("_([0-9]{1,3})_", "\n\\1-", gsub("^.*?NL-", "", NL_plasmid))), aes(mutant, NL_plasmid, fill=tstat_bret50)) + geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif deletion") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.x=element_blank()) +
...
p1
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/MotifValidation_IQCB1.tiff", units="cm", width=12, height=5, res=800)
p1
dev.off()
...
motif_maxBRET %>% filter(NL_symbol=="SPOP") %>% select(pval_cBRET)
```

```{r Figure3_Graph_variants}
# Heatmap 
p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="CTBP1" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(NL_symbol, mutant, fill=tstat_bret50  )) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank()) +
...
p2 <- ggplot(mutants_maxBRET %>% filter(NL_symbol=="CTBP1" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(NL_symbol, mutant, fill=tstat_cBRET  )) + 
  geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank()) +
...
grid.arrange(p1, p2,nrow=2, heights = c(1, 1), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_CTBP1_motif.tiff", units="cm", width=8, height=12, res=800)
grid.arrange(p1, p2,nrow=2, heights = c(1, 1), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()

p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="SPOP" & domain_variant==TRUE), aes(mutant, mCit_symbol, fill=tstat_bret50  )) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-30, 30), na.value = "grey50")+
  ggtitle("BRET50") + xlab("Domain variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank()) +
...
p1
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_SPOP_domain.tiff", units="cm", width=8, height=6, res=800)
p1
dev.off()


p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="WWOX" & domain_variant==TRUE), aes(mutant, mCit_symbol, fill=tstat_bret50  )) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-15, 15), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Domain variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(), axis.title.x=element_blank(),
...
p2 <- ggplot(mutants_maxBRET %>% filter(NL_symbol=="WWOX" & domain_variant==TRUE), aes(mutant, mCit_symbol, fill=tstat_cBRET  )) + 
  geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-15, 15), na.value = "grey50")+
  ggtitle("maxBRET") + xlab("Domain variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.y=element_blank(),
        axis.text.x = element_text(angle = 30, vjust = 1, hjust=1)) +
  scale_y_discrete(limits=rev)
grid.arrange(p1, p2, nrow=2, heights = c(4,5))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_WWOX_domain.tiff", units="cm", width=10, height=16, res=800)
grid.arrange(p1, p2, nrow=2, heights = c(4,5))
dev.off()

p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="WWOX" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(NL_symbol, mutant, fill=tstat_bret50  )) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-5, 5), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p2 <- ggplot(mutants_maxBRET %>% filter(NL_symbol=="WWOX" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(NL_symbol, mutant, fill=tstat_cBRET  )) + 
  geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-5, 5), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        ...
        axis.ticks = element_line(size = 1), axis.title.x=element_blank(), axis.title.y=element_blank()) +
  scale_y_discrete(limits=rev)
grid.arrange(p1, p2, nrow=2, heights = c(5,4), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_WWOX_motif.tiff", units="cm", width=7, height=15, res=800)
grid.arrange(p1, p2, nrow=2, heights = c(5,4), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()



p1 <- ggplot(mutants_maxBRET %>% filter(NL_symbol=="PPP3CA" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(NL_symbol, mutant, fill=tstat_cBRET  )) + 
  geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-25, 25), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif variant") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 1), axis.title.x=element_blank()) +
...
p1
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_PPP3CA_motif.tiff", units="cm", width=8, height=5, res=800)
p1
dev.off()

p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="IQCB1" & domain_variant==TRUE), aes(mCit_symbol, mutant, fill=tstat_bret50)) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
...
p2 <- ggplot(mutants_maxBRET %>% filter(NL_symbol=="IQCB1" & domain_variant==TRUE), aes(mCit_symbol, mutant, fill=tstat_cBRET)) + 
  geom_tile() + geom_text(aes(label=sig_maxBRET)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("maxBRET") + ylab("Motif variants") +
  theme_minimal() + labs(fill = "Effect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        ...
        axis.ticks = element_line(size = 1), axis.title.x=element_blank(),axis.title.y=element_blank()) +
  scale_y_discrete(limits=rev)
grid.arrange(p1, p2, nrow=2, heights = c(4,3), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_IQCB1_motif.tiff", units="cm", width=8, height=9, res=800)
grid.arrange(p1, p2, nrow=2, heights = c(4,3), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()

p1 <- ggplot(mutants_BRET50 %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), aes(mutant,NL_symbol , fill=tstat_bret50)) + 
  geom_tile() + geom_text(aes(label=sig_BRET50)) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey50")+
  ggtitle("BRET50") + xlab("Domain variants") +
  theme_minimal() + labs(fill = "Effect size") + 
...
tiff("Figures/Figure3_graphics/Heatmaps/VariantTest_IQCB1_domain.tiff", units="cm", width=16, height=7, res=800)
p1
dev.off()
...
```{r Figure4_PlotF}
ggplot(BRET_mutants, aes(y=mutation_location, fill=as.factor(interaction))) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
  ylab("Fraction DMIs perturbed") + xlab("Mutation location") + 
  theme_minimal() + scale_fill_manual(values=c("darkgrey","#006AB4")) + labs(fill = "Interaction") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position="none", 
        axis.ticks = element_line(size = 0.5), panel.grid.minor = element_blank())  + 
  scale_y_discrete(limits=rev, labels = rev(strc_categories)) 

p1 <- ggplot(BRET_mutants, aes(y=structural_region, fill=as.factor(interaction))) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_regions_noLITAF2.tiff", units="cm", width=12, height=8, res=800)
p1
dev.off()
...
p2 <- ggplot(BRET_mutants %>% select(aa_change, structural_region, interaction) %>% unique(), aes(y=structural_region, fill=as.factor(interaction))) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
...
p3 <- ggplot(BRET_mutants %>% select(aa_change, structural_region, interaction,AlphaMissense) %>% unique(), aes(y=structural_region, fill=AlphaMissense)) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
  ylab("Structural region") + xlab("Fraction AlphaMissense") + 
...
p4 <- ggplot(BRET_mutants %>% select(aa_change, structural_region, interaction, ClinVar) %>% unique(), aes(y=structural_region, fill=ClinVar)) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
  ylab("Structural region") + xlab("Fraction ClinVar") + 
...
p1 <- ggplot(scores_count, aes(x=bin, y=count_dmi)) +  geom_line() + 
  ...
  theme_classic() + theme(text = element_text(size = 20), axis.ticks = element_line(linewidth = 1)) + 
...
p1 <- ggplot(Model_metrics_fragments %>% filter(Fragment!="C"), aes(x=Fragment, y=avg_pLDDT, color=Fragment)) + 
  ...
  theme_classic() +  theme(text = element_text(size = 20), legend.position = "none", axis.ticks = element_line(linewidth = 1))
...
p2 <-ggplot(Model_metrics_fragments %>% filter(Fragment!="C"), aes(x=Fragment, y=avg_Acc, color=Fragment)) + 
  ...
  scale_color_manual(values=colors_DMI) + 
  theme_classic() +  theme(text = element_text(size = 20), legend.position = "none", aaxis.ticks = element_line(linewidth = 1))
...
p2a <- ggplot(DMI_AF_mut_hq %>% filter(structural_region!="Other" & Pathogenicity=="Uncertain"), aes(y=structural_region, fill=AM_pathogenicity)) + 
  ...
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position="none", panel.grid.minor = element_blank())  + 
  scale_y_discrete(limits=rev, labels = rev(strc_categories)) 
