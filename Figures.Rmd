...
temp <- HURI_ClinVar_DMI_mut_count_DMI_long %>% select(DomainProtein, DomainID1, DomainMatch1, SLiMProtein, Accession, SLiMMatch) %>% unique() %>% mutate(Domain_s=as.numeric(gsub("-.*?$","",DomainMatch1)), Domain_e=as.numeric(gsub(".*?-","",gsub("\\|","",DomainMatch1))), Motif_s=as.numeric(gsub("-.*?$","",SLiMMatch)), Motif_e=as.numeric(gsub("^.*?-","",SLiMMatch))) %>% select(-DomainMatch1, -SLiMMatch) %>% mutate(Domain_l=Domain_e-Domain_s, Motif_l=Motif_e-Motif_s) 
mean(temp$Domain_l) #207

HURI_ClinVar_mutation %>% filter(fragment_type=="Domain") %>% nrow() 
HURI_ClinVar_DMI_mut_count_DMI_long %>% select(DomainProtein, DomainID1, DomainMatch1) %>% unique() %>% nrow()
14103/561  #25 mutations per domain
14103/561/207 # 0.12 mutations per domain residue
mean(temp$Motif_l) #5.5
HURI_ClinVar_mutation %>% filter(fragment_type=="Motif") %>% nrow() 
HURI_ClinVar_DMI_mut_count_DMI_long %>% select(SLiMProtein, Accession, SLiMMatch) %>% unique() %>% nrow()
3045/5944 # 0.5 mutations per motif
3045/5944/5.5 #0.09 mutations per motif residue
```

```{r Figure1_Data2_PPI_VariantCount}
...
#length(HURI_ClinVar_DMI_mut_count_long_U)

HURI_ClinVar_DMI_mut_count_long %>% filter(pathogenicity=="Uncertain" | pathogenicity=="Pathogenic") %>% select(intx_ID) %>% unique() 
```
...
```{r Figure1_Supp_Graphic_ChiSq_pLDDT}
...
```{r Figure1_Supp_Graphic_IUPred}

p1 <- ggplot(ClinVar_mut_dis, aes(x=avg_IUPred, y=pathogenicity, color=as.factor(pathogenicity))) + 
  geom_quasirandom(size=0.5, alpha=0.4) + geom_boxplot(color="black", width=0.25, outlier.alpha = 0.2, alpha=0.5) +
  geom_vline(xintercept = 0.4 , size=0.5, color="red") + 
  ggtitle("ClinVar variant IUPred") + ylab("Clinical relevance") + xlab("Residue IUPred") + 
  theme_classic()  +  scale_color_manual(values=colors_Var) +
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), legend.position = "none", 
        axis.ticks = element_line(linewidth = 1)) + scale_y_discrete(limits=rev) +
...
#SAVE PNG
png("Figures/SuppFig_ClinVar_graphics/ClinVar_IUPred.png", units="cm", width=11, height=8, res=800)
p1
dev.off()
# #SAVE PDF
...
```{r Figure1_Supp_Graphic_ChiSq_IUPred}
### Correlations plots - IUPred classification

Pathogenicity_IUPred_chisq$residuals
Pathogenicity_IUPred_chisq$observed
corrplot(Pathogenicity_IUPred_chisq$stdres/(sd(Pathogenicity_IUPred_chisq$residuals)), method = 'color', is.corr  = FALSE, col = rev(COL2('RdBu')),  col.lim= c(-2.5, 2.5), cl.pos = 'b', addCoef.col = 'grey50', number.cex = 1, outline=T, addgrid.col="white")

corrplot(Pathogenicity_IUPred_chisq$stdres/(sd(Pathogenicity_IUPred_chisq$residuals)), method = 'color', is.corr  = FALSE, col = rev(COL2('RdBu')),  cl.pos = 'b', addCoef.col = 'grey50', number.cex = 1, outline=T, addgrid.col="white")
alpha_val <- 0.05 #significance
matrix_dim <- 8 # number of cells
qnorm(1-(alpha_val/matrix_dim)/2)

abs(Pathogenicity_IUPred_chisq$stdres/(sd(Pathogenicity_IUPred_chisq$residuals))) > qnorm(1-(alpha_val/length(Pathogenicity_IUPred_chisq$residuals))/2)
...
abs(Pathogenicity_IUPred_PU_chisq$stdres/(sd(Pathogenicity_IUPred_PU_chisq$residuals))) > qnorm(1-(alpha_val/length(Pathogenicity_IUPred_PU_chisq$residuals))/2)
...
p1 <- ggplot(ClinVar_mut_dis %>% filter(!is.na(AM_pathogenicity)), aes(y=AM_pathogenicity, x=avg_pLDDT, color=as.factor(AM_pathogenicity))) + 
  geom_quasirandom(size=0.5, alpha=0.4) + geom_boxplot(color="black", width=0.25, outlier.alpha = 0.2, alpha=0.5) +
  geom_vline(xintercept = 70 , size=0.5, color="red") + 
  ggtitle("AM ClinVar variants pLDDT") + ylab("AM pathogenicity") + xlab("Residue pLDDT") + 
  theme_classic()  +  scale_color_manual(values=colors_Var) +
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), legend.position = "none", 
        axis.ticks = element_line(linewidth = 1)) + scale_y_discrete(limits=rev) 
p1
#SAVE PNG
png("Figures/SuppFig_ClinVar_graphics/AM_pLDDT.png", units="cm", width=11, height=8, res=800)
p1
dev.off()
# #SAVE TIFF
...
p2 <- ggplot(ClinVar_mut_dis%>% filter(!is.na(AM_pathogenicity)), aes(y=AM_pathogenicity, x=avg_IUPred, color=as.factor(AM_pathogenicity))) + 
  geom_quasirandom(size=0.5, alpha=0.4) + geom_boxplot(color="black", width=0.25, outlier.alpha = 0.2, alpha=0.5) +
  geom_vline(xintercept = 0.4 , size=0.5, color="red") + 
  ggtitle("AM ClinVar variant pLDDT") + ylab("AM pathogenicity") + xlab("Residue IUPred") + 
  theme_classic()  +  scale_color_manual(values=colors_Var) +
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), legend.position = "none", 
        axis.ticks = element_line(linewidth = 1)) + scale_y_discrete(limits=rev) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 3))
p2
#SAVE PNG
png("Figures/SuppFig_ClinVar_graphics/AM_IUPred.png", units="cm", width=11, height=8, res=800)
p2
dev.off()
# #SAVE TIFF
...
p1 <- ggplot(HuRI_DMIs %>% filter(DMIMatchScore >= 0.7) %>% select(Accession, ELM_class, ELM_type) %>% unique(), aes(y = ELM_type, fill = ELM_type)) + 
  geom_bar( width = 0.5) + 
  ylab("Motif category") + xlab("Class number") + 
...
# SAVE TIFF
tiff("Figures/Figure2_graphics/HuRI_human_ClinVar_mutations_fragment.tiff", units="cm", width=9, height=8, res=800)
p1
dev.off()
# SAVE PDF
...
corrplot(Fragment_contingency_chisq$stdres, method = 'color', is.cor = FALSE,col = rev(COL2('RdBu')), col.lim= c(-36,36), cl.pos = 'b', addCoef.col = 'grey50', outline=T, addgrid.col="white")
...
grid.arrange(p1, p2, heights=c(4,5), nrow=2, bottom=textGrob("CTBP1 mutants", gp = gpar(fontsize = 20)) )
dev.off()
...
grid.arrange(p1, p2,nrow=2, bottom=textGrob("SPOP mutants", gp = gpar(fontsize = 20)))

# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/DomainValidation_SPOP.tiff", units="cm", width=7.5, height=13, res=800)
grid.arrange(p1, p2, heights=c(2.2,3), nrow=2, bottom=textGrob("SPOP mutants", gp = gpar(fontsize = 20)) )
dev.off()
...
grid.arrange(p1, p2, nrow=2, bottom=textGrob("WWOX mutants", gp = gpar(fontsize = 20)))

# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/DomainValidation_WWOX.tiff", units="cm", width=8, height=21, res=800)
grid.arrange(p1, p2, heights=c(4,5), nrow=2, bottom=textGrob("WWOX mutants", gp = gpar(fontsize = 20)))
dev.off()
...
grid.arrange(p1, p2, nrow=2, bottom=textGrob("PPP3CA mutants", gp = gpar(fontsize = 20)))

# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/DomainValidation_PPP3CA.tiff", units="cm", width=8, height=13, res=800)
grid.arrange(p1, p2, heights=c(2.2,3), nrow=2, bottom=textGrob("PPP3CA mutants", gp = gpar(fontsize = 20)))
dev.off()
# # SAVE TIFF
...
p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="IQCB1") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")) , 
             aes(label, NL_symbol, fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
p2 <- ggplot(domain_BRET %>% filter(NL_symbol=="IQCB1") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")) , 
             aes(label, NL_symbol, fill=tstat_cBRET)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") + theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
grid.arrange(p1, p2, widths=c(4.4,2), nrow=1, left=textGrob("Motif deletions", gp = gpar(fontsize = 20), rot=90) )
dev.off()
# # SAVE TIFF
...
p2 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% filter(!(mCit_symbol=="LITAF" & residue>20)) %>%
             ...
             aes(fct_reorder(label, residue), NL_symbol,  fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") +  xlab("Motif deletions") + theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none", 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/MotifValidation_WWOX_maxBRET.tiff", units="cm", width=12, height=7, res=800)
p2
dev.off()
...
p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% filter(mCit_symbol=="LITAF" & residue>20) %>%
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder(label, residue), NL_symbol,  fill=tstat_bret50  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none", 
...
p2 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% filter(mCit_symbol=="LITAF" & residue>20) %>%
  ...
  facet_grid2(.~category, scales = "free_x", switch = "x", space="free",
                 strip = strip_themed(text_x = element_text(size=0),
                                     background_x = elem_list_rect( fill =colors_BRET[2:3]  )))
grid.arrange(p1, p2, nrow=2, bottom=textGrob("Motif deletions", gp = gpar(fontsize = 20) ))

# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/MotifValidation_WWOX_maxBRET_LITAF.tiff", units="cm", width=12, height=7, res=800)
p2
dev.off()


p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="PPP3CA") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")),
...
p2 <- ggplot(motif_BRET %>% filter(NL_symbol=="PPP3CA") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
...
p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="IQCB1") %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder(fct_reorder2(mCit_symbol, residue, mCit_symbol), residue), label, fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
p2 <- ggplot(motif_BRET %>% filter(NL_symbol=="IQCB1") %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")),
             aes(fct_reorder2(mCit_symbol, residue, mCit_symbol), label, fill=tstat_cBRET)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") + theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/MotifValidation_IQCB1_BRETmax.tiff", units="cm", width=9, height=5, res=800)
p2
dev.off()
```
...
grid.arrange(p1, p2, heights=c(2,2.4), nrow=2, bottom=textGrob("SPOP variant", gp = gpar(fontsize = 20) ))
dev.off()
# # SAVE TIFF
...
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/VariantTest_WWOX_domain.tiff", units="cm", width=8, height=16, res=800)
grid.arrange(p1, p2, nrow=2, heights=c(2,2.5), bottom=textGrob("WWOX  variants", gp = gpar(fontsize = 20)))
dev.off()
# # SAVE TIFF
...
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/VariantTest_WWOX_motif_oneLITAF.tiff", units="cm", width=9, height=10, res=800)
grid.arrange(p1, p2, nrow=1, widths=c(3,1.7), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()
...
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/VariantTest_WWOX_motif_LITAF2.tiff", units="cm", width=9, height=8, res=800)
grid.arrange(p1, p2, nrow=1, widths=c(3,2), left=textGrob("Motif variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()

p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="WWOX" & motif_variant==TRUE) %>% 
               filter(mCit_symbol=="LITAF" & residue>20) %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder2(label, residue, mCit_symbol), NL_symbol, fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") +  xlab("Motif variants") +theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(),
        axis.text.x=element_text(angle = 30, vjust = 1, hjust=1), plot.title=element_text(size=15, hjust=0.95)) +
  facet_grid2(.~ClinVar, scales = "free_x", switch = "x",
                 strip = strip_themed(text_x = element_text(size=0),
                                     background_x = elem_list_rect( fill ="#999999" )))
                
p2
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/VariantTest_WWOX_motif_BRETmax_LITAF2.tiff", units="cm", width=10, height=7, res=800)
p2
dev.off()



p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="PPP3CA" & motif_variant==TRUE) %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(NL_symbol, fct_reorder(label, residue), fill=tstat_bret50  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
...
p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="PPP3CA" & motif_variant==TRUE) %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(NL_symbol, fct_reorder(label, residue), fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") + theme_minimal() + labs(fill = "maxBRET\neffect size") +
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder(label,  mCit_symbol),  NL_symbol, fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n"))
             , aes(fct_reorder(label,  mCit_symbol), NL_symbol, fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("maxBRET") + theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & domain_variant==TRUE) %>% 
             ...
             aes(mCit_symbol, fct_reorder(canonical_mutation, residue), fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  ggtitle("BRET50") +  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "none",
        ...
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(), axis.title.x=element_blank(),
        axis.text.x = element_blank(), plot.title=element_text(size=15, hjust=0.95)) +
  scale_y_discrete(limits=rev) + scale_x_discrete(limits=c("MNS1","CALM1","CALML3")) +
  facet_grid2(ClinVar~., scales = "free_y", switch = "y", space = "free_y", 
                 strip = strip_themed(text_y = element_text(size=0),
                                     background_y = elem_list_rect( fill ="#999999"))) 
...
grid.arrange(p1, p2, nrow=2, left=textGrob("IQCB1 variants", gp = gpar(fontsize = 20), rot=90 ))
# SAVE TIFF
tiff("/Users/jesusav/Desktop/Heatmaps/VariantTest_IQCB1_motif.tiff", units="cm", width=9, height=10, res=800)
grid.arrange(p1, p2, nrow=2,  heights=c(2.7,4), left=textGrob("IQCB1 variants", gp = gpar(fontsize = 20), rot=90 ))
dev.off()
# # SAVE TIFF
...
```{r Figure2_Graph_ComplexHeatmap_draft, include=F}
...
```{r Figure4_Graph_E}
BRET_mutants_update_motif$exp_conclusion <- ordered(BRET_mutants_update_motif$exp_conclusion, levels = c("not_disrupting","disrupting"))

p1 <- ggplot(BRET_mutants_update_motif, aes(y=mutation_location, fill=as.factor(exp_conclusion))) + 
  geom_bar(position="fill", width = 0.6, color="black", size=0.4) +
  ylab("Motif region") + xlab("Fraction DMIs perturbed") + 
  theme_minimal() + scale_fill_manual(values=c("#CADCE8","#00548C")) + labs(fill = "Interaction") +
  theme(text = element_text(size = 20), axis.title.y=element_blank() , panel.grid.minor = element_blank(), 
        legend.position="none", axis.ticks.x = element_line(size = 0.5), panel.grid.major = element_blank(), 
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_results_2.tiff", units="cm", width=7, height=8, res=800)
p1
dev.off()
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_scores_BRET2.tiff", units="cm", width=11, height=7, res=800)
grid.arrange(p1, p2 , widths=c(6,5),  nrow=1)
dev.off()
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_regions_maxBRET.tiff", units="cm", width=10, height=9, res=800)
p2
dev.off()
...
temp+ guides(colour = guide_legend(override.aes = list(size=5)))
```

```{r Figure4_Graph_f}
BRET_mutants_update$exp_conclusion <- ordered(BRET_mutants_update$exp_conclusion, levels = c("not_disrupting","disrupting"))
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_regions_2.tiff", units="cm", width=11, height=8, res=800)
p1
dev.off()
...
p2 <- ggplot(BRET_mutants_update %>% select(aa_change, structural_region, exp_conclusion) %>% unique(), 
  ...
  theme_minimal() + scale_fill_manual(values=c("#CADCE8","#00548C")) + labs(fill = "Disruption") +
  theme(text = element_text(size = 20),  panel.grid.minor = element_blank(), legend.position="none",
        axis.ticks = element_line(size = 0.5), panel.grid.major = element_blank(), 
        axis.ticks.y = element_line(size = 0), axis.text = element_text(color = "black"))   + 
  scale_y_discrete(limits=rev, labels = rev(strc_categories)) 
...
# SAVE TIFF
tiff("Figures/Figure4_graphics/BRET_mutants_CV_2.tiff", units="cm", width=11, height=8, res=800)
p4
dev.off()
...
p_legend +  scale_fill_manual(values=colors_Var, guide = guide_legend(theme = theme(
...
# SAVE TIFF
tiff("Figures/SuppFig_BRET_variants/BRET_mutants_scores.tiff", units="cm", width=18, height=8, res=800)
grid.arrange(p1, p2, p3,  nrow=1)
dev.off()

# SAVE PDF
pdf("Figures/SuppFig_BRET_variants/BRET_mutants_scores.pdf", width=8, height=4)
grid.arrange(p1, p2, p3,  nrow=1)
dev.off()
```
...
p1 <- ggplot(Model_metrics_motif, aes(x=Motif_region, y=pLDDT, color=Motif_region)) + 
  ...
  geom_hline(yintercept = 80, size=0.5, color="red") +
...
# SAVE PNG
png("Figures/Figure5_graphics/Motif_regions_pLDDT.png", units="cm", width=7, height=8, res=800)
p1
dev.off()
...
p2 <-ggplot(Model_metrics_motif, aes(x=Motif_region, y=SC_norm_Acc, color=Motif_region)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = 0.36, size=0.5, color="red") +
...
# SAVE PNG
png("Figures/Figure5_graphics/Motif_regions_RSA.png", units="cm", width=7, height=8, res=800)
p2
dev.off()
...
p3 <-ggplot(Model_metrics_motif, aes(x=Motif_region, y=Acc_change, color=Motif_region)) + 
  ...
  geom_hline(yintercept = -0.05, size=0.5, color="red") +
...
# SAVE PNG
png("Figures/Figure5_graphics/Motif_regions_deltaRSA.png", units="cm", width=7, height=8, res=800)
p3
dev.off()

grid.arrange(p1, p2, p3, nrow=1)

graph_x <-  "Residue \u0394 RSA"
...
# SAVE TIFF
tiff("Figures/Figure5_graphics/ClinVar_modelRegion.tiff", units="cm", width=11, height=8, res=800)
p1
dev.off()
...
p2 <-ggplot(Model_metrics_fragments %>% filter(Fragment!="C"), aes(x=Fragment, y=avg_Acc, color=Fragment)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = 0.36, size=0.5, color="red") +
  ggtitle("Fragment accessibility") + xlab("Fragment") + ylab("Fragment\naverage RSA") + ylim(0.15,0.7) +
  scale_color_manual(values=colors_DMI) + 
  theme_classic() +  
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), 
        legend.position = "none", axis.ticks = element_line(linewidth = 1))+
...
# SAVE PNG
png("Figures/Figure5_graphics/Fragments_pLDDT_RSA.png", units="cm", width=18, height=9, res=800)
grid.arrange(p1, p2,nrow=1)
dev.off()
...
p3 <- ggplot(Model_metrics %>% filter(Fragment!="C") %>% group_by(DMI, Fragment) %>% summarise(avg_dRSA = mean(Acc_change, na.rm=TRUE)) %>%
               ungroup() %>% select(DMI, Fragment, avg_dRSA) %>% unique(), 
            aes(x=Fragment, y=avg_dRSA, color=Fragment)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = -0.05, size=0.5, color="red") +
  ggtitle("Fragment accessibility") + xlab("Fragment") + ylab("Fragment\naverage \u0394 RSA") + ylim(0.15,0.7) +
  scale_color_manual(values=colors_DMI) + 
  theme_classic() +  
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), 
        legend.position = "none", axis.ticks = element_line(linewidth = 1))+
...
# SAVE PNG
png("Figures/Figure5_graphics/Fragments_pLDDT_RSA_dRSA.png", units="cm", width=22, height=9, res=800)
grid.arrange(p1, p2, p3, nrow=1)
dev.off()

p1 <- ggplot(Model_metrics %>% filter(Fragment!="C"), aes(x=Fragment, y=pLDDT, color=Fragment)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = 80, size=0.5, color="red") +
  ggtitle("Fragment confidence") + xlab("Fragment") + ylab("Fragment\nresidue pLDDT") + 
  scale_color_manual(values=colors_DMI) + 
...
p2 <-ggplot(Model_metrics %>% filter(Fragment!="C"), aes(x=Fragment, y=Norm_Acc, color=Fragment)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = 0.36, size=0.5, color="red") +
  ggtitle("Fragment accessibility") + xlab("Fragment") + ylab("Fragment\nresidue RSA") + ylim(0.15,0.7) +
  scale_color_manual(values=colors_DMI) + 
  theme_classic() +  
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), 
        legend.position = "none", axis.ticks = element_line(linewidth = 1))+
...
p3 <- ggplot(Model_metrics %>% filter(Fragment!="C"), aes(x=Fragment, y=Acc_change, color=Fragment)) + 
  geom_quasirandom(size=0.5, alpha=0.5) + geom_boxplot(color="black", na.rm = TRUE, width=0.25, alpha=0.5, outlier.alpha = 0.5) +
  geom_hline(yintercept = -0.05, size=0.5, color="red") +
  ggtitle("Fragment accessibility") + xlab("Fragment") + ylab("Fragment\nresidue \u0394 RSA")  + ylim(0.15,0.7) +
  scale_color_manual(values=colors_DMI) + 
  theme_classic() +  
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), 
        legend.position = "none", axis.ticks = element_line(linewidth = 1))+
...
# SAVE PNG
png("Figures/Figure5_graphics/Fragments_residue_pLDDT_RSA_dRSA.png", units="cm", width=22, height=9, res=800)
grid.arrange(p1, p2, p3, nrow=1)
dev.off()
```

```{r Figure4_Graph_Supp_flanks}
...
p2a <- ggplot(DMI_AF_mut_hq %>% filter(structural_region!="Other" & Pathogenicity=="Uncertain"), aes(y=structural_region, fill=AM_pathogenicity)) + 
  ...
  theme(text = element_text(size = 20), axis.text = element_text(color = "black"), 
        axis.ticks = element_line(size = 0.5), panel.grid.major = element_blank(), 
        axis.ticks.y = element_line(size = 0), legend.position="none", panel.grid.minor = element_blank())  + 
  scale_y_discrete(limits=rev, labels = rev(strc_categories)) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 3)) 
...
```{r Figure5_Graph_Larger_network}
