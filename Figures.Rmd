...
BRET_results <- BRET_results %>% mutate(include=case_when(NL_symbol=="IQCB1" &  mCit_symbol=="MNS1" & project_id=="Lu153r01" ~ "1",
                                                          NL_symbol=="SPOP" &  mCit_symbol=="MYD88" & project_id=="Lu171r01" ~ "1",
                                                          NL_symbol=="SPOP" &  mCit_symbol=="RXRB" & project_id=="Lu168r01" ~ "1", TRUE ~ include))
...
motif_BRET <- BRET_results %>% filter(mut_type=="Motif validation") %>% 
  ...
  mutate(residue=fct_inseq(gsub("[A-Z]", "", gsub("_.*?$", "",gsub("^.*?_", "", mutant))))) %>% # Mutant index
  mutate(tstat_bret50=case_when(tstat_bret50 < -20 ~ -20, tstat_bret50 > 20 ~ 20, TRUE ~ tstat_bret50), 
         tstat_cBRET=case_when(tstat_cBRET < -20 ~ -20, tstat_cBRET > 20 ~ 20, TRUE ~ tstat_cBRET)) # Capping values
...
p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant)) , 
  ...
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        ...
        legend.key.width= unit(0.2, 'cm'), legend.text = element_text(size=7, vjust=1)) +
  scale_y_discrete(limits=rev)

# library(ggh4x)
# temp <- domain_BRET %>% filter(NL_symbol=="CTBP1") %>% mutate(mutant=gsub("CTBP1_", "", mutant))
# p1 <- ggplot(temp , aes(fct_reorder(mutant, residue), mCit_symbol, fill=tstat_bret50  )) + 
#   geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
#   scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
#   xlab("CTBP1 domain mutants") +
#   theme_minimal() + labs(fill = "BRET50\neffect size") +
...
# p1 +facet_grid2(mCit_symbol~., scales = "free_y",  space = "free_y", switch = "y",
#                strip = strip_themed(text_y = element_text(size=0), background_y = elem_list_rect( fill = rainbow(length(unique(temp$mCit_symbol)))))) + theme(panel.spacing = unit(5, "lines"))

# p1 + facet_grid2(mCit_symbol~., scales = "free_y", switch = "y", 
#                  strip = strip_themed(text_y = element_text(size=0),
#                                      background_y = elem_list_rect( fill = rainbow( length( unique(temp$mCit_symbol)))))) 
#```
#```{r split}
...
p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="IQCB1") %>% mutate(mCit_plasmid=gsub("_", "\n", gsub("^.*?His3C-", "", mCit_plasmid))) , aes(mCit_plasmid, NL_symbol, fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p2 <- ggplot(motif_BRET %>% filter(NL_symbol=="SPOP") %>% mutate(mutant=gsub("_([0-9]{1,3})_", "\n\\1-", mutant)), 
             aes( NL_symbol, fct_reorder2(mutant, residue, desc(mCit_symbol)), fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% mutate(mutant=gsub("_", " ", gsub("_([0-9]{1,3})_", "\n\\1-", mutant))), 
             aes( NL_symbol, fct_reorder2(mutant, residue, desc(mCit_symbol)), fill=tstat_bret50  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="SPOP" & domain_variant==TRUE) %>% mutate(mutant=gsub("SPOP\n", "", mutant)), 
             aes(fct_reorder(mutant, residue), mCit_symbol, fill=tstat_bret50  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "BRET50\neffect size") +
...
p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="SPOP" & domain_variant==TRUE) %>% mutate(mutant=gsub("SPOP\n", "", mutant)), 
             aes(fct_reorder(mutant, residue), mCit_symbol, fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "maxBRET\neffect size") +
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="WWOX" & domain_variant==TRUE) %>% 
               mutate(mutant=gsub("WWOX\n", "", mutant)), aes(fct_reorder(mutant, residue), mCit_symbol, fill=tstat_bret50  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="WWOX" & domain_variant==TRUE) %>% 
               mutate(mutant=gsub("WWOX\n", "", mutant)) , aes(fct_reorder(mutant, residue), mCit_symbol, fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant)), 
             aes(fct_reorder2(mutant,  mCit_symbol, residue),  NL_symbol, fill=tstat_bret50)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_BRET50), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "BRET50\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        ...
        legend.key.width= unit(0.2, 'cm'), legend.text = element_text(size=7, vjust=1)) +
  scale_y_discrete(limits=rev)
p2 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% mutate(mutant=gsub(" ", "\n", mutant))
             , aes(fct_reorder2(mutant,  mCit_symbol, residue), NL_symbol, fill=tstat_cBRET  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-20, 20), na.value = "grey80")+
  theme_minimal() + labs(fill = "maxBRET\neffect size") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & domain_variant==TRUE) %>% 
               mutate(mutant=gsub("IQCB1\n", "", mutant), residue=as.numeric(gsub("[A-Z]", "", mutant))), 
