...
```{r Palettes}
## Different standardized color palettes 
...
```{r Figure1_rawData}
# Load ClinVar, ELM and HuRI datasets
setwd("/Volumes/imb-luckgr/Luck_lab_members/Jesus/DMIs_ClinVar/Datasets/")
ClinVar_mutations <- read_csv(file="All_DMIs-All_mutations/DDD_and_clinvar_mutations_2023_release.csv", col_names = T) #  all ClinVar variants
elm_instances <- read_tsv(file="ELM/ELM_Interacting_Regions_instances.tsv", col_names = T) #  ELM instances and classes
HuRI_DMIs <- read_csv(file="HuRI_DMIs-ClinVar_mutations/HuRI_human.csv", show_col_types = F) #  only HuRI human ones
HuRi_DMI <- read_csv(file="HuRI_DMIs-ClinVar_mutations/HuRI_human_highScores.csv") #highscored HuRI data
HURI_ClinVar_mutation <- read_csv(file="HuRI_DMIs-ClinVar_mutations/DMI_human_HuRI_overlap_clinvar_2023_release.csv", show_col_types = F) # variants overlap on DMI fragments

# Load human proteome disorder scores and AM variant scores
setwd("/Volumes/imb-luckgr/imb-luckgr2/projects/AlphaFold/AFDB_HumanReferenceProteome/")
Disorder_pLDDT <- read_tsv(file="proteins_pLDDT_values_byResidue.txt", col_names = T) # per residue protein pLDDT disorder scores
Disorder_IUPred <- read_tsv(file="CANO_ISO_SEQUENCES_IUPRed_LONG_disorder_byResidue.txt", col_names = T) # per residue protein IUPred disorder scores
AM_ClinVar_proteins <- read_tsv(file="AlphaMissense/Chunks/DMI_AM_Complete.tsv", col_names = F) # AM values for ClinVar variants from the DMI predictions
#The reference AM table could be extended to include all ClinVar proteins and not only the one from the DMI predictions;

# Load all RRS data
setwd("/Volumes/imb-luckgr/projects/dmi_predictor/ML_items/RRS/")
RRS1_ROC <- read_tsv(file="RRSv1/RRSv1_avg_ROC_scores.txt", col_names = T, skip = 1)
RRS2_ROC <- read_tsv(file="RRSv2/RRSv2_avg_ROC_scores.txt", col_names = T, skip = 1)
RRS3_ROC <- read_tsv(file="RRSv3/RRSv3_avg_ROC_scores.txt", col_names = T, skip = 1)
RRS4_ROC <- read_tsv(file="RRSv4/RRSv4_avg_ROC_scores.txt", col_names = T, skip = 1)
RRS1_PRC <- read_tsv(file="RRSv1/RRSv1_avg_PR_scores.txt", col_names = T, skip = 1)
RRS2_PRC <- read_tsv(file="RRSv2/RRSv2_avg_PR_scores.txt", col_names = T, skip = 1)
RRS3_PRC <- read_tsv(file="RRSv3/RRSv3_avg_PR_scores.txt", col_names = T, skip = 1)
RRS4_PRC <- read_tsv(file="RRSv4/RRSv4_avg_PR_scores.txt", col_names = T, skip = 1)

```

```{r Figure1_Data_Tables}
# Reduce ClinVar table info to only unique protein variants
ClinVar_mutations <- ClinVar_mutations %>% 
  select(uniprot_id, gene_symbol, aa_change, aa_pos, pathogenicity) %>% unique() # Keep columns with relevant information (excluding nucleotide information)
ClinVar_mutations$pathogenicity <- toTitleCase(ClinVar_mutations$pathogenicity) # Capitalize pathogenicity categories
ClinVar_mut_dis <- ClinVar_mutations %>% 
  rename(UniProt_ID = uniprot_id, Residue_n = aa_pos) %>% # Rename columns
...
Disorder_IUPred <- Disorder_IUPred %>% rename(UniProt_ID=ID, Residue_n=Residue_N)
...
Pathogenicity_pLDDT_dt <- as.table(as.matrix(Pathogenicity_pLDDT_table[,2:3])) # Value columns
rownames(Pathogenicity_pLDDT_dt) <- Pathogenicity_pLDDT_table$pathogenicity # Row names
Pathogenicity_pLDDT_chisq <- chisq.test(Pathogenicity_pLDDT_dt) # Chi-square test
...
Pathogenicity_IUPred_table <- ClinVar_mut_dis %>% 
  filter(!is.na(dis_IUPred)) %>% count(pathogenicity, dis_IUPred) %>% 
  mutate(Category=case_when(dis_IUPred==1 ~ "Disorder", dis_IUPred==0 ~ "Order")) %>% 
  select(-dis_IUPred) %>% pivot_wider(names_from = Category, values_from = n, values_fill=0 ) 

Pathogenicity_IUPred_dt <- as.table(as.matrix(Pathogenicity_IUPred_table[,2:3])) # Value columns
rownames(Pathogenicity_IUPred_dt) <-Pathogenicity_IUPred_table$pathogenicity # Row names
Pathogenicity_IUPred_chisq <- chisq.test(Pathogenicity_IUPred_dt) # Chi-square test

Pathogenicity_IUPred_PU_dt <- Pathogenicity_IUPred_dt[3:4,] # Only VUS and Pathogenic
Pathogenicity_IUPred_PU_chisq <- chisq.test(Pathogenicity_IUPred_PU_dt) # Chi-square test

rm(Pathogenicity_pLDDT_table, Pathogenicity_IUPred_table)
```

```{r Figure1_SupData_AlphaMissense}
# Edit table column and category names
colnames(AM_ClinVar_proteins) <- c("UniProt_ID", "AA_change","AM_value", "AM_pathogenicity")
AM_ClinVar_proteins <- AM_ClinVar_proteins %>% 
  mutate(AM_pathogenicity=case_when(AM_pathogenicity=="benign" ~ "Benign",
...
```{r Figure1_Data_DMIs}
# DMI predictions to get all ELM classes and types
...
HuRI_DMI_scores <- HuRI_DMIs %>% 
  select(intx_ID, SLiMProtein, Accession, SLiMMatch, DomainProtein, DomainID1, DomainMatch1, DMIMatchScore)

HuRI_DMI_scores %>% filter(DMIMatchScore >= 0.7) %>% nrow() # 12,957 DMIs with a score of 0.7 or higher
HuRI_DMI_scores %>% filter(DMIMatchScore >= 0.7) %>% select(intx_ID) %>% unique() %>% nrow() # 3,188 PPIs with high-scored DMIs
(HuRI_DMI_scores %>% filter(DMIMatchScore >= 0.7) %>% nrow()) /( HuRI_DMI_scores %>% nrow()) # 9.4% of DMI predictions are high-scored
#HuRi_DMI %>% select(Accession) %>% unique()
```

```{r Figure1_Data_RRS}
# Combine all RRS data
RRS1_ROC$RRS <- "RRS1"
RRS2_ROC$RRS <- "RRS2"
RRS3_ROC$RRS <- "RRS3"
RRS4_ROC$RRS <- "RRS4"
...
# Count number of DMIs based on an increasing threshold filter
...
```{r Figure1_ELMclasses_annotations}
...
```{r Figure1_Data_HURI_ClinVar_mutation}
# ClinVar variants overlap on DMI fragments
...
# High-scored HuRI data
HuRi_DMI <- HuRi_DMI %>% filter(DMIMatchScore>=0.7) # Double-check score filtering 
...
```{r Figure2_rawData}
setwd("/Volumes/imb-luckgr/Luck_lab_members/Jesus/DMIs_ClinVar/Datasets/BRET_experiments/")

BRET_results <- read_tsv(file="titration_cat_scoring_v6_with_include.tsv")  # BRET result statistical analysis
mutant_descriptions <- read_csv(file="mutant_description.csv") # Mutant information
BRET_mutants <- read_tsv(file="tested_disease_mutations_structure_computations_extended.txt") # 

# Load titration data
BRET_selection <- read_csv(file="titrations_filtering.csv") # Selected BRET experiments
BRET_plasmid_layout <- read_csv(file="plate_layout.csv") # Plate layout
BRET_values <- read_csv(file="titration_values.csv") # BRET values
BRET_fits <- read_csv(file="titration_fit.csv") # BRET curve fits
```

```{r Figure2_data_BRET}
#Edit BRET results table
...
domain_BRET <- domain_BRET %>% select(mCit_plasmid,NL_symbol, mCit_symbol, pval_bret50, tstat_bret50, pval_cBRET, tstat_cBRET, sig_BRET50, sig_maxBRET, BRET_category, mutant, residue, BRET50_Log2_FChng, maxBRET_Log2_FChng)
...
motif_BRET <- BRET_results %>% filter(mut_type=="Motif validation") %>% 
  ...
  mutate(tstat_bret50=case_when(tstat_bret50 < -20 ~ -20, tstat_bret50 > 20 ~ 20, TRUE ~ tstat_bret50), 
         tstat_cBRET=case_when(tstat_cBRET < -20 ~ -20, tstat_cBRET > 20 ~ 20, TRUE ~ tstat_cBRET)) # Capping values
motif_BRET <- motif_BRET %>% select(NL_plasmid, NL_symbol, mCit_symbol, pval_bret50, tstat_bret50, pval_cBRET, tstat_cBRET, sig_BRET50, sig_maxBRET, BRET_category, mutant, residue, BRET50_Log2_FChng, maxBRET_Log2_FChng)
...
mutants_BRET <- BRET_results %>% filter(exp_category=="variant") %>% 
  ...
  mutate(tstat_bret50=case_when(tstat_bret50 < -20 ~ -20, tstat_bret50 > 20 ~ 20, TRUE ~ tstat_bret50), 
         tstat_cBRET=case_when(tstat_cBRET < -20 ~ -20, tstat_cBRET > 20 ~ 20, TRUE ~ tstat_cBRET)) # Capping values
mutants_BRET <- mutants_BRET %>% select(NL_plasmid, mCit_plasmid,NL_symbol, mCit_symbol, pval_bret50, tstat_bret50, pval_cBRET, tstat_cBRET, sig_BRET50, sig_maxBRET, BRET_category, motif_variant, domain_variant, mutant, residue, BRET50_Log2_FChng, maxBRET_Log2_FChng)
...
mutants_BRET %>% count(motif_variant, domain_variant)
...
BRET_mutants <- read_tsv(file="tested_disease_mutations_structure_computations_extended.txt")
BRET_mutants_update <- read_tsv(file="tested_disease_mutations_structure_computations_extended_2.tsv")
```

```{r Figure3_data1}

BRET_mutants <- BRET_mutants %>% 
  mutate(AlphaMissense=case_when(AlphaMissense=="likely pathogenic"  ~ "Pathogenic",
                                 AlphaMissense=="uncertain"  ~ "Ambiguos",
                                 AlphaMissense=="likely benign"  ~ "Benign", TRUE ~ AlphaMissense))

BRET_mutants <- BRET_mutants %>% 
  mutate(Fragment=case_when(mutation_location=="domain"  ~ "Domain",
                            mutation_location=="outside motif" |  mutation_location=="core motif" ~ "Motif"))

BRET_mutants <- BRET_mutants %>% 
  mutate(interface = case_when(pLDDT >= 80 & deltaRSA<(-0.05)  ~ "interface", TRUE ~ "not_interface"))  %>% 
...
BRET_mutants$structural_region <- ordered(BRET_mutants$structural_region, 
                                          levels = c("d_interface", "d_buried","d_surface", "m_interface","m_n_interface"))  
BRET_mutants$AlphaMissense <- ordered(BRET_mutants$AlphaMissense, 
                                      levels = c("Benign", "Ambiguos","Pathogenic")) 

BRET_mutants <- BRET_mutants %>% 
  filter(!(motif_partner=="LITAF" & (aa_change=="P58L" |aa_change=="P59R" |aa_change=="Y61C"  )))

BRET_mutants_motif <-  BRET_mutants %>% 
  filter(mutation_location!="domain") %>% 
  mutate(mutation_location=case_when(mutation_location=="core motif" ~ "Core",
...
BRET_mutants_update <- BRET_mutants_update %>% mutate(exp_conclusion=case_when(pval_cBRET <= 0.05 ~ "disrupting", TRUE ~ "not_disrupting")) 

BRET_mutants_update <- BRET_mutants_update %>% 
  mutate(AlphaMissense=case_when(AlphaMissense=="likely pathogenic"  ~ "Pathogenic",
                                 AlphaMissense=="uncertain"  ~ "Ambiguos",
                                 AlphaMissense=="likely benign"  ~ "Benign", TRUE ~ AlphaMissense))
...
BRET_mutants_update$structural_region <- ordered(BRET_mutants_update$structural_region, 
                                                 levels = c("d_interface", "d_buried","d_surface", "m_interface","m_n_interface"))  
BRET_mutants_update$AlphaMissense <- ordered(BRET_mutants_update$AlphaMissense, 
                                             levels = c("Benign", "Ambiguos","Pathogenic")) 
# BRET_mutants_update_motif <-  BRET_mutants_update %>% 
#   filter(mutation_location!="domain") %>%
#   mutate(mutation_location=case_when(mutation_location=="core motif" ~ "Core", 
#                                      mutation_location=="outside motif" ~ "Flank"))

BRET_mutants_update %>% filter(mCit_symbol=="RXRB")
BRET_mutants_update %>% count(fragment_variant)

BRET_results <- BRET_results %>% 
  mutate(aa_changeA=gsub(mCit_symbol,"",gsub("^.*?_","", gsub("pcDNA3.1 cmyc-NL-", "", NL_plasmid)))) %>%
  mutate(aa_changeB=gsub(mCit_symbol,"",gsub("^.*?_","", gsub("pcDNA3.1 mCit-His3C-", "", mCit_plasmid)))) %>%
  mutate(aa_change=case_when(aa_changeA==NL_symbol ~ aa_changeB, aa_changeB==mCit_symbol ~ aa_changeA, aa_changeB=="" ~ aa_changeA)) %>%
  select(-aa_changeA, -aa_changeB)

BRET_mutants_update  <- left_join(BRET_mutants_update %>% mutate(tstat_cBRET=as.character(round(tstat_cBRET,5))), 
                                  BRET_results %>% mutate(tstat_cBRET=as.character(round(tstat_cBRET,5))) %>% 
                                    select(NL_symbol, mCit_symbol,tstat_cBRET, maxBRET_Log2_FChng)
                                  ) 
...
BRET_mutants%>% filter(motif_partner=="LITAF") %>% arrange(aa_change) %>% select(aa_change, mutation_location, structural_region, AlphaMissense, RSA, deltaRSA, pLDDT)
```


```{r Figure4_rawData}
setwd("/Volumes/imb-luckgr/imb-luckgr2/projects/AlphaFold/HuRI_DMI_AF_modelling/")

DMI_AF_metrics <- read_csv(file="Score_files/DMI_AF_model_metrics.csv")
DMI_AF_mutations <- read_tsv(file="Score_files/DMI_AF_model_metrics_mappedMutations.tsv")

Model_metrics  <- read_tsv(file="All_model_metrics.tsv")
Model_metrics_fragments <- read_tsv(file="All_model_metrics_byFragments.tsv")

Scores_json_raw <- read_tsv(file="Score_files/DMI_model_confidence_all_2.tab", col_names = T, na = "")
...
```{r Figure5_rawData}
setwd("/Volumes/imb-luckgr/imb-luckgr2/projects/AlphaFold/AFDB_HumanReferenceProteome/GeneCode_012025/")
GeneSymbols <- read.csv(file="GeneSymbols_Uniprot.csv")

setwd("/Volumes/imb-luckgr/Luck_lab_members/Jesus/DMIs_ClinVar/Datasets/")
Protein_disorders_child_parents <- read_csv(file="AF_model_tables/DO_disease_parent_terms_filled.csv")
HuRI_GO <- read_tsv(file="AF_model_tables/HuRI_proteins_gene_GOterms.tsv")

HuRI_essentiality <- read_csv(file="HuRI_paper_tables/HuRI_Essentiality.csv")
HuRI_age <- read_csv(file="HuRI_paper_tables/HuRI_Age.csv")
HuRI_exp <- read_csv(file="HuRI_paper_tables/HuRI_Exp_GTEx.csv")
HuRI_LoFint <- read_csv(file="HuRI_paper_tables/HuRI_LoFint.csv")

HURI_ClinVar_mut_motif_2 <- read_tsv(file="HuRI_DMIs-ClinVar_mutations/DMI_human_HuRI_overlap_clinvar_2023_release_MotifDisruption.tsv")
HURI_ClinVar_mut_motif_pLDDT <- read_tsv(file="HuRI_DMIs-ClinVar_mutations/DMI_human_HuRI_overlap_clinvar_2023_release_MotifpLDDT.tsv")
...
p1 <- ggplot(ClinVar_mutations, aes(y=pathogenicity , fill=pathogenicity)) + geom_bar(width = 0.6) +
  ...
  theme(text = element_text(size = 15), axis.text = element_text(color = "black"), legend.position = "none",  
        axis.ticks = element_line(linewidth = 1, color = "black"), axis.text.x = element_text(angle = 30, vjust = 1, hjust=1)) + 
...
#SAVE TIFF
tiff("Figures/Figure1_graphics/DMI_predictor_ROC.tiff", units="cm", width=in2cm(4), height=in2cm(4), res=800)
p1
dev.off()
#SAVE PDF
...
#SAVE TIFF
tiff("Figures/Figure1_graphics/DMI_predictor_PRC.tiff", units="cm", width=in2cm(4), height=in2cm(4), res=800)
p2
dev.off()
#SAVE PDF
...
p1 <- ggplot(HuRI_DMIs %>% filter(DMIMatchScore >= 0.7), aes(y = ELM_type, fill = ELM_type)) + geom_bar(width = 0.7) + 
  ...
  theme(text = element_text(size = 18), legend.position="none", 
        axis.text = element_text(color = "black"), axis.ticks = element_line(linewidth = 1, color = "black"), 
...
p1 <- ggplot(HURI_ClinVar_mutation, aes(x=fragment_type , fill=pathogenicity)) + 
  ...
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), legend.position = "null", 
        ...
        axis.ticks.y = element_line(linewidth = 0.5, color = "black"), axis.title.x=element_blank())+
...
#SAVE PNG
png("Figures/ExtendedFigure_1/ClinVar_pLDDT.png", units="cm", width=11, height=8, res=800)
p1
dev.off()
# #SAVE PDF
...
```{r ED_Figure1_Graphic_c_ROC_PR_comparisons}
comparisons <- read_csv("/Users/jesusav/Desktop/RF_testing/Datasets/ROC_PR_RandomForest.csv")
comparisons <- comparisons %>% mutate(RF=as.factor(RF))
RF_labs <- c("All",  "IUPred", "IUPred & RLC", "RLC")

p1 <- ggplot(comparisons %>% filter(RF!="Nprop"), aes(x=mean_fpr, y=mean_tpr, color=RF)) +  geom_line(size=1) + 
  xlab("1 - Specificity") + ylab("Sensitivity") + 
  theme_classic() + 
  theme(text = element_text(size = 25), axis.text = element_text(color = "black"), 
        axis.ticks = element_line(color = "black")) + 
  labs(color = "Features")  + scale_color_discrete(labels=RF_labs)+
  scale_x_continuous(breaks=c(0,0.5,1)) + scale_y_continuous(breaks=c(0,0.5,1)) 
p1

#SAVE PDF
pdf("/Users/jesusav/Desktop/RF_testing/Plots/RF_comparison_ROC.pdf", width=6.5, height=4)
p1
dev.off()

p2 <- ggplot(comparisons %>% filter(RF!="Nprop"), aes(x=mean_recall, y=mean_precision, color=RF)) +  geom_line(size=1) +
...
#SAVE PDF
pdf("/Users/jesusav/Desktop/RF_testing/Plots/RF_comparison_PRC.pdf", width=6.5, height=4)
p2
dev.off()
```

```{r ED_Figure1_Graphic_d_ROC_PR_holdouts}
holdouts <- read_csv("/Users/jesusav/Documents/IMB_Luck/DMIs_ClinVar/Manuscript/RF_testing/Datasets/ROC_PR_Holdouts.csv")
holdouts <- holdouts %>% mutate(RF=as.factor(RF))
RF_labs <- c("Complements", "Intersect")

p1 <- ggplot(holdouts , aes(x=mean_fpr, y=mean_tpr, color=RF)) +  geom_line(size=1) + 
  #annotate(geom="text", label=c("Complements AUC = 0.92", "Intersect AUC = 0.92"), x=0.3, y=c(0.1,0.04), hjust=0, size=4, color="black") +
  xlab("1 - Specificity") + ylab("Sensitivity") + 
  theme_classic() + scale_color_discrete(labels=RF_labs) +
  theme(text = element_text(size = 25), axis.text = element_text(color = "black"), 
        axis.ticks = element_line(color = "black")) + 
  labs(color = "Sets")  + 
  scale_x_continuous(breaks=c(0,0.5,1)) + scale_y_continuous(breaks=c(0,0.5,1)) 
...
#SAVE PDF
pdf("/Users/jesusav/Desktop/RF_testing/Plots/RF_holdouts_ROC.pdf", width=6.5, height=4)
p1
dev.off()

p2 <- ggplot(holdouts , aes(x=mean_recall, y=mean_precision, color=RF)) +  geom_line(size=1) +
  #annotate(geom="text", label=c("Complements AP = 0.88", "Intersect AP = 0.94"), x=0.1, y=c(0.4,0.35), hjust=0, size=4, color="black") +
  xlab("Recall") + ylab("Precision") + 
  theme_classic() + 
  theme(text = element_text(size = 25), axis.text = element_text(color = "black"), 
        axis.ticks = element_line(color = "black")) + 
  labs(color = "Sets") + scale_color_discrete(labels=RF_labs) +
  scale_x_continuous(breaks=c(0,0.5,1)) 
p2

#SAVE PDF
pdf("/Users/jesusav/Desktop/RF_testing/Plots/RF_holdouts_PRC.pdf", width=6.5, height=4)
p2
dev.off()
```

```{r ED_Figure1_Graphic_e_DMI_scores}
...
p1
# SAVE TIFF
tiff("Figures/ExtendedFigure_1/DMIScore_cummulative_DMI_PPI.tiff", units="cm", width=12, height=8, res=800)
p1
dev.off()
# SAVE PDF
pdf("Figures/ExtendedFigure_1/DMIScore_cummulative_DMI_PPI.pdf", width=cm2in(12), height=cm2in(8))
p1
dev.off()
...
p2 <- ggplot(DMIscores_count, aes(x=bin, y=count_dmi)) +  geom_line() + 
  ...
  theme_classic() + 
  theme(text = element_text(size = 20), axis.line = element_line(size = 1), 
        axis.text = element_text(color = "black"), axis.ticks = element_line(linewidth = 1, color = "black")) + 
...
```{r ED_Figure1_Data_f_Multivalency_categories}
HuRI_mult <- HuRI_DMIs %>% count(intx_ID, Accession, DomainID1)  %>% rename(DMI_n=n) %>% 
  group_by(intx_ID, DomainID1) %>% summarise(M_n=length(Accession), D_DMI_n=sum(DMI_n)) %>% ungroup()  %>% 
  group_by(intx_ID) %>% summarise(D_n=length(DomainID1), D_M_n=sum(M_n), DM_DMI_n=sum(D_DMI_n)) %>% ungroup() %>% 
  arrange(-D_n, -DM_DMI_n)


HuRI_mult_hq <- HuRI_DMIs %>% filter(DMIMatchScore>=0.7) %>% 
  count(intx_ID, Accession, DomainID1)  %>% rename(DMI_n=n) %>% 
  group_by(intx_ID, DomainID1) %>% summarise(M_n=length(Accession), D_DMI_n=sum(DMI_n)) %>% ungroup()  %>% 
  group_by(intx_ID) %>% summarise(D_n=length(DomainID1), D_M_n=sum(M_n), DM_DMI_n=sum(D_DMI_n)) %>% ungroup() %>% 
  arrange(-D_n, -DM_DMI_n)

HuRI_mult %>% filter(DM_DMI_n==1) #1,787 PPIs 20.1%, 1,787 DMIs 1,2%  
HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n==1) %>% select(DM_DMI_n) %>% sum() # 1,353 PPIs 15.89%, 6,044 DMIs 4.39% 
HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==1) %>% select(DM_DMI_n) %>% sum() # 
# 3,077 PPIs are mediated by multiple DMIs, from multiple motifs but from a single domain type 36.15% of PPIs, 33,230 DMIs 24%
HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==2) %>% select(DM_DMI_n) %>% sum() # 2,083 PPIs 24.47%,85,959 DMIs 62.38%
HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n>2) %>% select(DM_DMI_n) %>% sum() # 211 PPIs 2.47%, 10,760 DMIs 7.8%

HuRI_mult_table <- tibble(category=c("1D_1M_1DMI", "1D_1M_mDMI", "1D_mM_mDMI","2D_mM_mDMI","mD_mM_mDMI"),
                          PPI_all=c(HuRI_mult %>% filter(DM_DMI_n==1) %>% nrow(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n==1) %>% nrow(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==1) %>% nrow(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==2) %>% nrow(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n>2) %>% nrow()),
                          
                          DMI_all=c(HuRI_mult %>% filter(DM_DMI_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==2)%>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n>2) %>% select(DM_DMI_n) %>% sum()),
                          
                          PPI_hq=c(HuRI_mult_hq %>% filter(DM_DMI_n==1) %>% nrow(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n==1) %>% nrow(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==1) %>% nrow(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==2) %>% nrow(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n>2) %>% nrow()),
                          
                          DMI_hq=c(HuRI_mult_hq %>% filter(DM_DMI_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==1) %>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n==2)%>% select(DM_DMI_n) %>% sum(),
                                HuRI_mult_hq %>% filter(DM_DMI_n>1 & D_M_n>1 & D_n>2) %>% select(DM_DMI_n) %>% sum()
                                ))
HuRI_mult_table

HuRI_mult_table_ln <- HuRI_mult_table %>% 
  mutate(PPI_lq=PPI_all-PPI_hq, DMI_lq=DMI_all-DMI_hq) %>% select(-PPI_all, -DMI_all) %>%
  pivot_longer(cols = PPI_hq:DMI_lq, names_to = c("type"), values_to = "number") %>% 
  separate(type, c("c_type", "quality"), sep="_") %>%
  pivot_wider(names_from=c("c_type"), values_from = number)

HuRI_mult_table_ln
```

```{r ED_Figure1_Graphic_f_Multivalency_categories_piegraph}
p1 <- ggplot(HuRI_mult_table, aes(x="", y=PPI_all, fill=category)) +
  geom_bar(stat="identity", width=1) +
  scale_fill_manual(values=colors_blues) +
  coord_polar("y", start=0) + theme_void() + theme(legend.position="none")
p1

#SAVE PDF
pdf("/Users/jesusav/Desktop/DMIs_combinations_PPIs.pdf", width=4, height=4)
p1
dev.off()

p2 <- ggplot(HuRI_mult_table, aes(x="", y=DMI_all, fill=category)) +
  geom_bar(stat="identity", width=1) +
  scale_fill_manual(values=colors_blues) +
  coord_polar("y", start=0) + theme_void() + theme(legend.position="none")
p2
...
#SAVE PDF
pdf("/Users/jesusav/Desktop/DMIs_combinations_DMIs.pdf", width=4, height=4)
p2
dev.off()

p1 <- ggplot(HuRI_mult_table, aes(x="", y=PPI_hq, fill=category)) +
  geom_bar(stat="identity", width=1) +
  scale_fill_manual(values=c(colors_blues[1:4], "#006AB4")) +
  coord_polar("y", start=0) + theme_void() + theme(legend.position="none")
p1

#SAVE PDF
pdf("/Users/jesusav/Desktop/DMIs_combinations_PPIs_hq.pdf", width=4, height=4)
p1
dev.off()

p2 <- ggplot(HuRI_mult_table, aes(x="", y=DMI_hq, fill=category)) +
  geom_bar(stat="identity", width=1) +
  scale_fill_manual(values=colors_blues) +
  coord_polar("y", start=0) + theme_void() + theme(legend.position="none")
p2
...
#SAVE PDF
pdf("/Users/jesusav/Desktop/DMIs_combinations_DMIs_hq.pdf", width=4, height=4)
p2
dev.off()

(HuRI_mult_table$PPI_hq)/sum(HuRI_mult_table$PPI_hq)
(HuRI_mult_table$DMI_hq)/sum(HuRI_mult_table$DMI_hq)
```

```{r ED_Figure1_Graphic_g_DMI_scores_categories}
...
#SAVE png
png("Figures/ExtendedFigure_1/HuRI_DMI_Scores.png", units="cm", width=11, height=8, res=800)
p1
dev.off()
# SAVE PDF
...
```{r ED_Figure1_Graphic_h_DMI_classes_categories}
...
# SAVE TIFF
tiff("Figures/ExtendedFigure_1/HuRI_DMI_highScore_Classes_fraction.tiff", units="cm", width=11, height=8, res=800)
p1
dev.off()

# SAVE PDF
pdf("Figures/ExtendedFigure_1/HuRI_DMI_highScore_Classes_fraction.pdf", width=5, height=3)
p2
dev.off()

```

```{r ED_Figure1_Graphic_i_DMI_known_instances}
...
# SAVE png
png("Figures/ExtendedFigure_1/Supp_HuRI_DMI_scores_instances.png", units="cm", width=8, height=8, res=800)
p1
dev.off()
...
```{r ED_Figure1_Graphic_j_DMI_ClinVar_overlaps}
...
# SAVE TIFF
tiff("Figures/ExtendedFigure_1/HuRI_DMIs_OneMutation.tiff", units="cm", width=11, height=8, res=800)
p1
dev.off()
# SAVE PDF
pdf("Figures/ExtendedFigure_1/HuRI_DMIs_OneMutation.pdf", width=cm2in(11), height=cm2in(8))
p1
dev.off()
...
# SAVE TIFF
tiff("Figures/ExtendedFigure_1/HuRI_PPIs_OneMutation.tiff", units="cm", width=11, height=8, res=800)
p2
dev.off()

# SAVE PDF
pdf("Figures/ExtendedFigure_1/HuRI_PPIs_OneMutation.pdf", width=cm2in(11), height=cm2in(8))
p2
dev.off()

```

```{r ED_Figure1_Graphic_k_DMI_ClinVar_motif_categories}
#DMI - Motif variants
...
# SAVE TIFF
tiff("Figures/ExtendedFigure_1/HuRI_human_ClinVar_mutations_ELM.tiff", units="cm", width=11, height=8, res=800)
p1
dev.off()

# SAVE PDF
pdf("Figures/ExtendedFigure_1/HuRI_human_ClinVar_mutations_ELM.pdf", width=8, height=4)
p1
dev.off()
...
p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="CTBP1"), 
             aes(fct_reorder(canonical_mutation, residue), mCit_symbol, fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4, 4), na.value = "grey80")+
  xlab("CTBP1 mutants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/DomainValidation_CTBP1_FC_maxBRET.pdf", width=cm2in(12), height=cm2in(8))
p1
dev.off()


p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="SPOP"), 
             aes(fct_reorder(canonical_mutation, residue), mCit_symbol, fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4, 4), na.value = "grey80")+
  xlab("SPOP mutants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/DomainValidation_SPOP_FC_maxBRET.pdf", width=cm2in(10.3), height=cm2in(7.5))
p1
dev.off()



p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="WWOX"), 
             aes(fct_reorder(canonical_mutation, residue), mCit_symbol, fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4, 4), na.value = "grey80")+
  xlab("WWOX mutants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(), axis.title.x =  element_text(hjust=0.3),
        axis.text.x = element_text(angle = 30, vjust = 1, hjust=1), plot.title = element_blank(),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/DomainValidation_WWOX_FC_maxBRET.pdf", width=cm2in(11.3), height=cm2in(11))
p1
dev.off()

p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="PPP3CA"), 
             aes(fct_reorder(canonical_mutation, residue), mCit_symbol, fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4, 4), na.value = "grey80")+
  xlab("PPP3CA mutants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/DomainValidation_PP3CA_FC_maxBRET.pdf", width=cm2in(10.8), height=cm2in(7.5))
p1
dev.off()


p1 <- ggplot(domain_BRET %>% filter(NL_symbol=="IQCB1") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")) , 
             aes(NL_symbol, label, fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4, 4), na.value = "grey80")+
  theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/DomainValidation_IQCB1_FC_maxBRET.pdf", width=cm2in(8.8), height=cm2in(5.5))
p1
dev.off()
...
p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="CTBP1") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(NL_symbol, fct_reorder2(label, as.numeric(residue), desc(mCit_symbol)), fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("Motif deletions") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_CTBP1_FC_maxBRET.pdf", width=cm2in(10.3), height=cm2in(5.5))
p1
dev.off()


p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="SPOP") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(NL_symbol, fct_reorder(label, residue), fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("Motif deletions") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_SPOP_FC_maxBRET.pdf", width=cm2in(10.3), height=cm2in(11))
p1
dev.off()
...
p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
  ...
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("Motif mutations") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.x=element_blank(), 
        plot.title=element_text(size=12, hjust=0.95),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_WWOX_FC_maxBRET.pdf", width=cm2in(10.3), height=cm2in(16))
p1
dev.off()


p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% filter(!(mCit_symbol=="LITAF" & residue>20)) %>%
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder(label, residue), NL_symbol,  fill=maxBRET_Log2_FChng  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  xlab("Motif deletions") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(),
        axis.text.x = element_text(angle = 30, vjust = 1.1, hjust=1), plot.title=element_blank(),
        legend.position = "none") +
  facet_grid2(.~category, scales = "free_x", switch = "x", space="free",
                 strip = strip_themed(text_x = element_text(size=0),
                                     background_x = elem_list_rect( fill =colors_BRET[2:3]  )))

p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_WWOX_FC_maxBRET_deletions.pdf", width=cm2in(13), height=cm2in(7))
p1
dev.off()


p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="WWOX") %>% filter(mCit_symbol=="LITAF" & residue>20) , 
             aes(NL_symbol, fct_reorder(canonical_mutation, residue),  fill=maxBRET_Log2_FChng  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("LITAF mutations") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(),  
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.x=element_blank(),
        plot.title=element_text(size=12, hjust=0.95),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_WWOX_FC_maxBRET_LITAF.pdf", width=cm2in(8.8), height=cm2in(9))
p1
dev.off()

p1 <- ggplot(motif_BRET %>% filter(NL_symbol=="IQCB1") %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(label, fct_reorder(fct_reorder2(mCit_symbol, residue, mCit_symbol), residue), fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/MotifValidation_IQCB1_FC_maxBRET.pdf", width=cm2in(8.3), height=cm2in(7.5))
p1
dev.off()

domain_BRET %>% filter(NL_symbol=="IQCB1")
```
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="CTBP1" & motif_variant==TRUE) %>% mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")),
             aes(NL_symbol, fct_reorder2(label, residue, mCit_symbol), fill=maxBRET_Log2_FChng  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("Motif variants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_CTBP1_motif_FC_maxBRET.pdf", width=cm2in(9.3), height=cm2in(8))
p1
dev.off()

p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="SPOP" & domain_variant==TRUE) %>%
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(fct_reorder(label, residue), mCit_symbol, fill=maxBRET_Log2_FChng  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_SPOP_FC_maxBRET.pdf", width=cm2in(8.5), height=cm2in(7.5))
p1
dev.off()
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="WWOX" & domain_variant==TRUE),
  ...
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  xlab("WWOX variants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(), axis.title.x =  element_text(hjust=0.5),
        axis.text.x = element_text(angle = 30, vjust = 1, hjust=1), plot.title = element_blank(),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_WWOX_domain_FC_maxBRET.pdf", width=cm2in(11.3), height=cm2in(9))
p1
dev.off()
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_WWOX_motif_FC_maxBRET.pdf", width=cm2in(9.3), height=cm2in(12))
p1
dev.off()
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_WWOX_motif_FC_maxBRET_oneLITAF.pdf", width=cm2in(9.3), height=cm2in(8))
p1
dev.off()

p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="WWOX" & motif_variant==TRUE) %>% 
               filter(mCit_symbol=="LITAF" & residue>20), 
             aes(NL_symbol, fct_reorder2(canonical_mutation, residue, mCit_symbol), fill=maxBRET_Log2_FChng  )) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("LITAF variants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), 
        panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.x=element_blank(), plot.title=element_blank(),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_WWOX_motif_FC_maxBRET_LITAF.pdf", width=cm2in(8.8), height=cm2in(7))
p1
dev.off()
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & motif_variant==TRUE) %>% 
               mutate(label=paste(gene_symbol,canonical_mutation, sep="\n")), 
             aes(NL_symbol, fct_reorder(label,  mCit_symbol),  fill=maxBRET_Log2_FChng)) + 
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  ylab("Domain variants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), 
        panel.grid.major = element_blank(), 
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.x=element_blank(), plot.title=element_blank(),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_IQCB1_domain_FC_maxBRET.pdf", width=cm2in(9.3), height=cm2in(12))
p1
dev.off()
...
p1 <- ggplot(mutants_BRET %>% filter(NL_symbol=="IQCB1" & domain_variant==TRUE) %>% 
  ...
  geom_tile(color = "white", lwd = 1.2) + geom_text(aes(label=sig_maxBRET), size=7) +
  scale_fill_continuous_divergingx(palette = heatmap_pltt, rev = TRUE, mid = 0, limits = c(-4,4), na.value = "grey80")+
  xlab("IQCB1 variants") + theme_minimal() + labs(fill = "Log2 FC\nmaxBRET") +
  theme(text = element_text(size = 20), 
        panel.grid.major = element_blank(),
        axis.text = element_text(color = "black"), panel.grid.minor = element_blank(), 
        axis.ticks = element_line(size = 0), axis.title.y=element_blank(), axis.title.x=element_text(hjust=0.8),
...
p1
# SAVE PDF
pdf("/Users/jesusav/Desktop/FoldChange/VariantTest_IQCB1_motif_FC_maxBRET.pdf", width=cm2in(9.8), height=cm2in(8))
p1
dev.off()


mutants_BRET %>% filter(NL_symbol=="SPOP" & domain_variant==TRUE) %>% select(mCit_plasmid, tstat_cBRET, pval_cBRET, maxBRET_Log2_FChng)
```

```{r Figure2_Graph_titrationPlots}
#BRET_mutant_pairs <- BRET_mutant_pairs %>% mutate(mCit_construct=case_when(mCit_construct=="MYD88_NA" ~ "MYD88_134_138del", TRUE ~ mCit_construct)) 
for(i in 1:nrow(BRET_mutant_pairs)){
  ...
  p <- ggplot() + geom_point(data=plot_data, size=2.2, aes(x=ratio, y = cBRET, color=test) ) + 
      ...
      theme(text = element_text(size = 18), axis.text = element_text(color = "black"), legend.position="none", 
            plot.title= element_text(size = 13), axis.ticks = element_line(linewidth = 1, color = "black")) +
  ...
  file_name <- paste("/Users/jesusav/Desktop/Titrations/PDF/",pair_info[5],"_",pair_info[6],".pdf",sep="")
    pdf(file_name, width=cm2in(8.15), height=cm2in(6))
    print(p)
    dev.off()
...
t.test(pLDDT ~ mutation_location, data = BRET_mutants_update_motif)
t.test(deltaRSA ~ mutation_location, data = BRET_mutants_update_motif)
...
p3 <- ggplot(BRET_mutants_update_motif, aes(x=mutation_location, y=maxBRET_Log2_FChng)) + 
  geom_quasirandom(size=2, alpha=1, aes(color=exp_conclusion)) + #geom_beeswarm(corral = "wrap", alpha=0.2, size=0.1) + 
  geom_boxplot(color="black",  alpha=0.1,  width=0.5, lwd=0.4, outlier.size = 0.5, outlier.alpha = 1) + 
  scale_color_manual(values=c("#00548C","#56B4E9")) +
  xlab("Group") + ylab("Log2 FC maxBRET") + ylim(-2.2,0.5) +
  theme_classic() + 
  theme(text = element_text(size = 20), legend.position="none", axis.text = element_text(color = "black"), 
        axis.title.x=element_blank(), plot.title=element_blank() ) +
  scale_x_discrete(labels = c("Core","Flank","Core","Flank")) 
t.test(maxBRET_Log2_FChng ~ mutation_location, data = BRET_mutants_update_motif)
...
# SAVE PDF
pdf("/Users/jesusav/Desktop/BRET_mutants_scores_FCmaxBRET_2.pdf",  width=cm2in(7), height=cm2in(8))
p3
dev.off()

BRET_mutants_update %>% select(mut_protein, aa_change, ClinVar, exp_conclusion) %>% unique() %>% count(ClinVar,exp_conclusion)
```

```{r Figure4_Graphic_e_BRET_structural_regions_FC}
BRET_mutants_update$structural_region <- ordered(BRET_mutants_update$structural_region, levels = c("d_buried","d_interface", "m_interface","d_surface","m_n_interface"))
p1 <- ggplot(BRET_mutants_update , aes(x=structural_region, y=maxBRET_Log2_FChng)) + 
  geom_quasirandom(size=2, alpha=1, aes(color=exp_conclusion)) + #geom_beeswarm(corral = "wrap", alpha=0.2, size=0.1) + 
  geom_boxplot(color="black", alpha=0.1, width=0.5, lwd=0.4, outlier.size = 1, outlier.alpha = 1) + 
  scale_color_manual(values=c("#00548C","#56B4E9")) +
  xlab("Group") + ylab("Log2 FC maxBRET") +  ylim(-2.2,0.4) +
  theme_classic() + 
  theme(text = element_text(size = 18), legend.position="none", axis.text = element_text(color = "black"), 
        axis.title.x=element_blank(), plot.title=element_blank(), 
        axis.text.x = element_text(angle = 30, vjust = 1, hjust=1)) +
  scale_x_discrete(labels = c("D buried","D interface", "M interface","D surface","M not int.")) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 3)) 
p1

# SAVE PDF
pdf("/Users/jesusav/Desktop/BRET_mutants_regions_FC_maxBRET.pdf", width=cm2in(10), height=cm2in(9))
p1
dev.off()

BRET_mutants_update %>% arrange(maxBRET_Log2_FChng)

BRET_mutants_update <- BRET_mutants_update %>% mutate(group=case_when(structural_region %in% c("d_surface","m_n_interface") ~ "non_disrp", TRUE ~ "disrp"))
t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update)
t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update)

t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update %>% filter(fragment_variant=="motif"))
t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update %>% filter(fragment_variant=="motif" & aa_change!="S41L" & tstat_cBRET!=-1.62866) ) 
t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update %>% filter(structural_region=="d_interface" | structural_region=="d_surface"))
t.test(maxBRET_Log2_FChng~ group, data = BRET_mutants_update %>% filter(structural_region=="d_buried" | structural_region=="d_surface"))

BRET_mutants_update %>% filter(structural_region=="m_n_interface") %>% arrange(-maxBRET_Log2_FChng)

ggplot(BRET_mutants_update , aes(x=maxBRET_Log2_FChng, y=-log10(pval_cBRET), color=exp_conclusion)) + geom_point() +
  geom_vline(xintercept =c(0.5, -0.5), size=0.25, color="dimgrey") +
  geom_hline(yintercept =-log10(0.05), size=0.25, color="dimgrey") +
  theme_classic() +
  scale_color_manual(values=c("#00548C","#56B4E9"))

BRET_mutants_update %>% filter(exp_conclusion=="disrupting" & pval_cBRET>0.05)

BRET_mutants_update %>% filter(structural_region=="m_interface")
```

```{r Figure4_Graphic_f_AM_structural_regions}
p1 <- ggplot(BRET_mutants_update %>% select(aa_change, structural_region, AlphaMissense) %>% unique(), 
...
# SAVE PDF
pdf("/Users/jesusav/Desktop/BRET_mutants_AM_2.pdf",  width=cm2in(11), height=cm2in(8))
p1
dev.off()
...
BRET_mutants_update %>% filter(structural_region=="m_n_interface") %>% select(NL_symbol, mCit_symbol, aa_change, exp_conclusion, ClinVar, AlphaMissense) %>% unique()
```
...
DMI_AF_mut_hq %>% count(structural_region, AM_pathogenicity)
```
...
```{r MoMaP}
interections <- c("Q13363_Q96MA1", "Q13363_Q13422", "P0DP23_Q15051", "P27482_Q15051", "Q08209_Q96KS9", "O43791_Q99836", "O43791_P28702", "Q16630_Q9NZC7", "Q15038_Q9NZC7", "Q99732_Q9NZC7")
All_DMIs %>% filter(intx_ID %in% interections) %>% select(intx_ID, SLiMProtein, Elm, SLiMMatch, Pattern, DomainProtein, DomainName1, DomainMatch1)


All_DMIs %>% filter(intx_ID == "Q16630_Q9NZC7") %>% select(intx_ID, SLiMProtein, Elm, SLiMMatch, Pattern, DomainProtein, DomainName1, DomainMatch1)
```